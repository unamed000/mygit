...
``info``, ``warning``, ``error``, ``critical`` in that order using a 0 - 5 
level.

Logger also provides the ability to globally share logs using a single api and 
with good practice control our systems entire log with a single constant
definition.
